...
model_name = lambda obj: obj.__name__.lower()

class Migration(DataMigration):

    def forwards(self, orm):
        BooleanQuestion = orm.BooleanQuestion
        RadioAnswer     = orm.RadioAnswer
        BooleanAnswer   = orm.BooleanAnswer
        ContentType     = orm['contenttypes.ContentType']

        for quest in BooleanQuestion.objects.all():
            for answer in RadioAnswer.objects.filter(question=quest.pk):
                ctype  = ContentType.objects.get(model=model_name(BooleanAnswer))
                kwargs = {
                    'user': answer.user,
                ...
                bool_answer = BooleanAnswer.objects.create(**kwargs)
                answer.delete()
